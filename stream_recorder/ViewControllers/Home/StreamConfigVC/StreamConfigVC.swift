...
class StreamConfigVC: UIViewController {
    ...
    enum ConfigRow: Int, CaseIterable {
        ...
        var title: String {
            ...
            case .bit_rate:
                return "Bit Rate"
            case .direction:
                return "Broadcast Direction"
            case .volumes:
                return "App & Mic Volumes"
            }
        }
        ...
        var data: String {
            ...
            case .direction:
                return Cache.shared.video_format.title
            default:
                return ""
            }
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let _ = touches.first?.view as? OptionsView {
            return
        } else {
...
extension StreamConfigVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.view.subviews.contains(where: {$0.isKind(of: OptionsView.self)}) {
            ...
            if let cell = tableView.cellForRow(at: indexPath) as? RecordConfigCell {
                switch item {
                case .resolution:
                    cell.showVideoOption(mode: .resolution, direction: .down, inView: self.view) {
                        tableView.reloadRows(at: [indexPath], with: .automatic)
                    }
                case .frame_rate:
                    cell.showVideoOption(mode: .frame, direction: .down, inView: self.view) {
                        tableView.reloadRows(at: [indexPath], with: .automatic)
                    }
                case .bit_rate:
                ...
                case .direction:
                    cell.showVideoOption(mode: .format, direction: .up, inView: self.view) {
                        tableView.reloadRows(at: [indexPath], with: .automatic)
                    }
                default:
                    break
