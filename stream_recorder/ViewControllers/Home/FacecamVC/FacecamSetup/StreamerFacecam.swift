...
class StreamerFacecam: NSObject {
    ...
    // mp4 record
    internal var fileWriter: AVAssetWriter?
    internal var videoInput: AVAssetWriterInput?
    internal var audioInput: AVAssetWriterInput?
    internal var pixelBufferInput: AVAssetWriterInputPixelBufferAdaptor?
    internal var isRecordSessionStarted = false
    internal var recordStatus: RecordFacecameState = .none
...
//            pipTransform?.orientation = orientation
    ...
    func stopRecord(restart: Bool = false, cancel: Bool = false) {
        DLog("stopRecord")
...
extension StreamerFacecam: AVCapturePhotoCaptureDelegate {
    @available(iOS 11.0,*)
...
// MARK: Video player
extension StreamerFacecam: AVPlayerItemOutputPullDelegate {
    func initVideo(_ videoURL: URL?) {
