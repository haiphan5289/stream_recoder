...
public class SOTabBar: UIView {
   ...
   internal var viewControllers = [UIViewController]() {
    ...
    private func drawConstraint() {
        ...
        outerCircleView.layer.cornerRadius = (innerCircleView.frame.size.height - 12) / 2
        ...
        var constraints = [
            ...
            outerCircleView.heightAnchor.constraint(equalToConstant: innerCircleView.frame.size.height - 12),
            outerCircleView.widthAnchor.constraint(equalToConstant: innerCircleView.frame.size.width - 12),
    ...
    private func didSelectTab(index: Int) {
        ...
        if index == 0 {
            UIView.animate(withDuration: SOTabBarSetting.tabBarAnimationDurationTime, delay: 0, options: [.layoutSubviews]) {
                self.outerCircleView.backgroundColor = UIColor(hex: "75b9f2")
            }
        } else if index == 1 {
            UIView.animate(withDuration: SOTabBarSetting.tabBarAnimationDurationTime, delay: 0, options: [.layoutSubviews]) {
                self.outerCircleView.backgroundColor = UIColor(hex: "a190d4")
            }
        } else {
            UIView.animate(withDuration: SOTabBarSetting.tabBarAnimationDurationTime, delay: 0, options: [.layoutSubviews]) {
                self.outerCircleView.backgroundColor = UIColor(hex: "ff758e")
            }
        }
    }
...
private extension SOTabBar {
    ...
    var circlePath: CGPath {
        ...
        let y = SOTabBarSetting.tabBarHeight * 0.25
